package com.example.horoscapp.iu.horoscope.adapter
...
class HoroscopeViewHolder( view: View):RecyclerView.ViewHolder(view) {
    ...
    fun render(horoscopeInfo: HoroscopeInfo, onItemSelected: (HoroscopeInfo) -> Unit){
        ...
        binding.imHoroscope.setImageResource(horoscopeInfo.img)
        binding.tvHoroscope.text = context.getString(horoscopeInfo.name)

        binding.parent.setOnClickListener {
            startRotationAnimation(binding.imHoroscope, newLambda = {onItemSelected(horoscopeInfo)})
